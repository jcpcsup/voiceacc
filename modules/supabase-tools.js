export function createSupabaseTools(api) {
  ...
  function serializeAccountForSupabase(account, userId, syncedAt, sortOrder = 0) {
    return {
      ...
      include_in_total_balance: account.includeInTotalBalance !== false,
  ...
  function serializeTransactionForSupabase(transaction, userId, syncedAt) {
    return {
      ...
      counterparty_amount: Number(transaction.counterpartyAmount || 0),
  ...
  function deserializeSupabaseAccount(row) {
    return {
      ...
      includeInTotalBalance: row.include_in_total_balance !== false,
  ...
  function deserializeSupabaseTransaction(row) {
    return {
      ...
      counterpartyAmount: Number(row.counterparty_amount || 0),
      project: row.project || "",
